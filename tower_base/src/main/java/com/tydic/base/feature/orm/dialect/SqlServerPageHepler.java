 ...
 **/
public class SqlServerPageHepler {
    /**
     * 得到查询总数的sql
     */
    ...
    public static String getLimitString(String sql, int offset, int limit) {
        ...
        // sqlArr[0] = sql.substring(0, sql.indexOf("from") + 4);
        // sqlArr[1] = sql.substring(sql.indexOf("from") + 4, sql.length());
        //
        // StringBuffer pagingSelect = new StringBuffer(sql.length() + 100);
        // pagingSelect.append(sqlArr[0]);
        // pagingSelect.append(" (select *,ROW_NUMBER() OVER (ORDER BY id) as
        // rank from");
        // pagingSelect.append(sqlArr[1]);
        // pagingSelect.append(")  as t where t.rank between " + (offset + 1) + " and " + (offset + limit));

        
        sql = sql.trim();
        String orderByContent = sql.substring(sql.indexOf("order by") + 8, sql.length());
        String selectContent = sql.substring(sql.indexOf("select") + 6, sql.indexOf("from"));
        String fromContent = sql.substring(sql.indexOf("from") + 4, sql.indexOf("where"));
        String whereContent = sql.substring(sql.indexOf("where") + 5, sql.indexOf("order by"));
        
        StringBuffer sqlBuffer = new StringBuffer("WITH OrderedOrders AS (select row_number() over(order by ");
        ...
        sqlBuffer.append(") select * from OrderedOrders WHERE rowNum between " + (offset + 1) + " and " + (offset + limit));
